...
namespace TombExtract
{
    class TR3Utilities
    {
        ...
        public TR3Utilities(IWin32Window owner)
        {
            this.owner = owner;
        }
                    ...
                    for (int i = 0; i < savegames.Count; i++)
                    {
                        ...
                        if (sourcePlatform == Platform.PC && destinationPlatform == Platform.PC)
                        {
                            if (isSourcePrepatch && isDestinationPatch5)    // PRE-PATCH -> PATCH 5
                            {
                                ...
                                for (int offset = currentSavegameOffset, j = 0; offset < currentSavegameOffset + DESTINATION_SAVEGAME_SIZE; offset++, j++)
                                {
                                    ...
                                    if (currentRelativeOffset >= 0x988 && currentRelativeOffset <= Globals.SAVEGAME_SIZE_TRX_PREPATCH)
                                    {
                                        destinationFile.Seek(offset + 0x10, SeekOrigin.Begin);
                            ...
                            else if (!isSourcePrepatch && !isDestinationPatch5)  // PATCH 5 -> PRE-PATCH
                            {
                                ...
                                for (int offset = currentSavegameOffset, j = 0; offset < currentSavegameOffset + DESTINATION_SAVEGAME_SIZE; offset++, j++)
                                {
                                    ...
                                    if (currentRelativeOffset >= 0x998 && currentRelativeOffset <= Globals.SAVEGAME_SIZE_TRX_PREPATCH)
                                    {
                                        destinationFile.Seek(offset - 0x10, SeekOrigin.Begin);
                                        destinationFile.Write(currentByte, 0, currentByte.Length);
                                    }
                        ...
                        else if (sourcePlatform.IsMobile() && destinationPlatform == Platform.PC) // Mobile -> PC
                        {
                            ...
                            for (int offset = currentSavegameOffset, j = 0; offset < currentSavegameOffset + DESTINATION_SAVEGAME_SIZE; offset++, j++)
                            {
                                ...
                                if (currentRelativeOffset >= 0x8A0 && currentRelativeOffset < 0x9DF)
                                {
                                    destinationFile.Seek(offset - 0x40, SeekOrigin.Begin);
                                    destinationFile.Write(currentByte, 0, currentByte.Length);
                                }
                                else if (currentRelativeOffset >= 0x9DF && currentRelativeOffset <= Globals.SAVEGAME_SIZE_TRX_PATCH5)
                                {
                                    destinationFile.Seek(offset - 0x4B, SeekOrigin.Begin);
                                    destinationFile.Write(currentByte, 0, currentByte.Length);
                                }
                                else
                                {
                        ...
                        else if (sourcePlatform == Platform.PC && destinationPlatform.IsMobile()) // PC -> Mobile
                        {
                            if (isSourcePrepatch && isDestinationPatch5)
                            {
                                ...
                                for (int offset = currentSavegameOffset, j = 0; offset < currentSavegameOffset + DESTINATION_SAVEGAME_SIZE; offset++, j++)
                                {
                                    ...
                                    if (currentRelativeOffset >= 0x8A0 && currentRelativeOffset < 0x994)
                                    {
                                        destinationFile.Seek(offset + 0x40, SeekOrigin.Begin);
                                        destinationFile.Write(currentByte, 0, currentByte.Length);
                                    }
                                    else if (currentRelativeOffset >= 0x994 && currentRelativeOffset <= Globals.SAVEGAME_SIZE_TRX_PATCH5)
                                    {
                                        destinationFile.Seek(offset + 0x4B, SeekOrigin.Begin);
                                        destinationFile.Write(currentByte, 0, currentByte.Length);
                                    }
        ...
        private byte[] ConvertNativePatch5EntityBlockToPrepatchFormat(byte[] source)
        {
            ...
            for (int itemIndex = 0; itemIndex < levelObjectIds.Count; itemIndex++)
            {
                ...
                if ((tr3Object.Flags00 & 0x20) != 0)
                {
                    ...
                    short aiWord = BitConverter.ToInt16(source, blockStart + 2);
                    ...
                    CopyBytes(source, destination, ref sourceCursor, ref destinationCursor, increment);
                }

                if ((tr3Object.Flags00 & 0x80) != 0)
                {
                    CopyBytes(source, destination, ref sourceCursor, ref destinationCursor, 0x04);
                }

                if (objectId == 0x12)
                {
                    CopyBytes(source, destination, ref sourceCursor, ref destinationCursor, 0x08);
                }

                if (objectId == 0x0F)
                {
                    CopyBytes(source, destination, ref sourceCursor, ref destinationCursor, 0x1C);
                }

                if (objectId == 0x0E)
                {
                    CopyBytes(source, destination, ref sourceCursor, ref destinationCursor, 0x30);
                }

                if (objectId == 0x11)
                {
                    CopyBytes(source, destination, ref sourceCursor, ref destinationCursor, 0x20);
                }

                if (objectId == 0x10)
                {
                    CopyBytes(source, destination, ref sourceCursor, ref destinationCursor, 0x2C);
                }
